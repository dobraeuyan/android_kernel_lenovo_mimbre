...
static struct thermal_zone thermal_zones[] = {
    ...
    {"max170xx_battery", -1, 64000},
    {"dollar_cove_battery", -1, 60000},
    {"STR0", -1, 80000},
    {"STR2", -1, 80000}
};
...
static void temperature_check()
{
    ...
    for (i = 0; i < sizeof(thermal_zones)/sizeof(struct thermal_zone); i++) {
        if (thermal_zones[i].index == -1) {
            continue;
        }
        zone_temp = get_zone_temp(thermal_zones[i].index);
        if (zone_temp < -20000 || zone_temp > 125000) {
            LOGW("Invalid temperature reported for zone %s", thermal_zones[i].name);
            continue;
        }
