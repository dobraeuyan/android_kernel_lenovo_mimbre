...
void wpa_supplicant_mark_disassoc(struct wpa_supplicant *wpa_s)
{
	...
	/*
	 * Intel regulatory rule:
	 * P2P GO activities on a non worldwide safe channel are authorized
	 * only if we are no connected as STA on such channel.
	 */
					 ...
					 struct wpa_bss *bss)
{
	...
	/*
	   Modify by Tk
	   wpa_supplicant_ssid_bss_match () fun has updated the struct wpa_scan_res to wpa_bss,
	   So use wpa_bss_get_ie() to instead of wpa_scan_get_ie
	*/
			  ...
			  int only_first_ssid)
{
	...
	/*
	   Modify by Tk
	   wpa_supplicant_select_bss() has updated the incoming parameters,
	   so use wpa_bss to instead of wpa_scan_res
	*/
	...
	*/
	for (i = 0; i < wpa_s->last_scan_res_used; i++)
	{
		...
		/*
		   Modify by Tk
		   wpa_bss_get_ie() instead of wpa_scan_get_ie()
		*/
	...
	if (selected)
	{
		/*
		   Modify by Tk
		   wpa_bss instead of wpa_scan_res
		*/
		struct wpa_bss *bss = selected;
		const u8 *ie, *ssid;
		u8 ssid_len;
		/*
		   Modify by Tk
					  ...
					  int locally_generated)
{
	...
	/*
	 * Intel regulatory rule:
	 * P2P GO activities on a non worldwide safe channel must be stopped
	 * as soon as we are no more connected as STA on such channel.
	 */
	if (wpa_s->assoc_freq > 2462 && wpa_s) {
		struct wpa_global *global = wpa_s->global;
		struct wpa_supplicant *curr = global->ifaces;
		struct wpa_supplicant *next;
		while (curr) {
			next = curr->next;
			if (curr->p2p_group_interface == P2P_GROUP_INTERFACE_GO
					   && curr->assoc_freq == wpa_s->assoc_freq) {
				wpa_msg(wpa_s, MSG_INFO,
						"STA connection ended on freq %d, "
						"force P2P GO disconnection", curr->assoc_freq);
				wpas_p2p_disconnect(curr);
			}
			curr = next;
		}
	}
