...
LOCAL_MODULE_PATH_32 := $(PRODUCT_OUT)$(HAL_BM_SOURCE_WIFI_PREFIX)/aosp
LOCAL_MODULE_PATH_64 := $(PRODUCT_OUT)$(HAL_BM_SOURCE_WIFI_PREFIX)/aosp
endif
...
ifeq ($(HAL_AUTODETECT),true)
LOCAL_MODULE_PATH_32 := $(PRODUCT_OUT)$(HAL_BM_SOURCE_WIFI_PREFIX)/aosp/lib
LOCAL_MODULE_PATH_64 := $(PRODUCT_OUT)$(HAL_BM_SOURCE_WIFI_PREFIX)/aosp/lib64
endif
LOCAL_CFLAGS = $(L_CFLAGS)
...
ifeq ($(HAL_AUTODETECT),true)
# Save LOCAL_PATH that is overwritten in sub-Android.mk files
PRV_LOCAL_PATH := $(LOCAL_PATH)
include $(PRV_LOCAL_PATH)/Android.rtl.mk
include $(PRV_LOCAL_PATH)/Android.brcm.mk
endif

ifeq ($(HAL_AUTODETECT),true)
# Create links for Autodetect
WPA_SUPPLICANT_AUTODETECT:
	$(hide) mkdir -p $(PRODUCT_OUT)/system/bin
	$(hide) mkdir -p $(PRODUCT_OUT)/system/lib
	$(hide) ln -sf $(HAL_BM_TARGET_WIFI)/wpa_supplicant $(PRODUCT_OUT)/system/bin/wpa_supplicant
	$(hide) ln -sf $(HAL_BM_TARGET_WIFI)/wpa_cli $(PRODUCT_OUT)/system/bin/wpa_cli
	$(hide) ln -sf $(HAL_BM_TARGET_WIFI)/lib/libwpa_client.so $(PRODUCT_OUT)/system/lib/libwpa_client.so
ifeq (x86_64, $(TARGET_ARCH))
	$(hide) mkdir -p $(PRODUCT_OUT)/system/lib64
	$(hide) ln -sf $(HAL_BM_TARGET_WIFI)/lib64/libwpa_client.so $(PRODUCT_OUT)/system/lib64/libwpa_client.so
endif
#	$(hide) ln -sf $(HAL_BM_TARGET_WIFI)/wpa_supplicant.conf $(PRODUCT_OUT)/system/etc/wifi/wpa_supplicant.conf
ALL_DEFAULT_INSTALLED_MODULES += WPA_SUPPLICANT_AUTODETECT
endif
