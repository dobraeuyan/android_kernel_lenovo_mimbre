...
void wpas_p2p_update_channel_list(struct wpa_supplicant *wpa_s)
{
	 ...
	 * Only world wide safe channels are authorized for P2P GO.
	 * If we are connected as a STA on another channel, this channel
	 * is also authorized while STA remains connected.
	 */

	// First find the STA channel
	u8 sta_chan = 0;
	for (ifs = wpa_s->global->ifaces; ifs; ifs = ifs->next) {
		if (!ifs->current_ssid || ifs->current_ssid->mode != WPAS_MODE_INFRA)
			continue;
		ieee80211_freq_to_chan(ifs->assoc_freq, &sta_chan);
		break;
	}
	if (sta_chan != 0)
		wpa_printf(MSG_DEBUG, "Connected as STA on channel %u", sta_chan);

	// Remove all unsafe channels
	size_t o, c;
	o = 0;
	while (o < chan.reg_classes) {
		...
		c = 0;
		while (c < op->channels) {
			if ((op->channel[c] < 1 || op->channel[c] > 11)
					   && op->channel[c] != sta_chan) {
				wpa_printf(MSG_DEBUG,
						"Remove unsafe channel %u", op->channel[c]);
				op->channels--;
