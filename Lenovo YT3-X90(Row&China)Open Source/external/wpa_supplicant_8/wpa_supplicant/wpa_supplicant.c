...
int wapi_merge_cert_files(char *wapi_as_cert, char *wapi_user_cert, char *cert_name)
{
	...
	if (wapi_as_cert && strncmp("keystore://", wapi_as_cert, 11) == 0) {
		if ((length = keystore_get(&wapi_as_cert[11], strlen(wapi_as_cert)-11, &pdata)) == -1) {
	...
	if (wapi_user_cert && strncmp("keystore://", wapi_user_cert, 11) == 0) {
		if ((length = keystore_get(&wapi_user_cert[11], strlen(wapi_user_cert)-11, &pdata)) == -1) {
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
	...
	if ( (ssid->wapi) || (ssid->proto & WPA_PROTO_WAPI) || (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_PSK ) ||
		...
		wpa_printf(MSG_DEBUG, "%s: params.freq.freq=%d.\n", __FUNCTION__, params.freq.freq);
...
int wpa_supplicant_update_mac_addr(struct wpa_supplicant *wpa_s)
{
	if (wpa_s->driver->send_eapol) {
		...
		l2_packet_deinit(wpa_s->l2);
#ifdef WAPI
		wpa_printf(MSG_DEBUG, "%s : wpa_s->driver->send_eapol flags %x, ifname:%s ",
		    __FUNCTION__, wpa_s->drv_flags, wpa_s->ifname);

		wpa_s->l2_wapi = l2_packet_init(wpa_s->ifname,
						wpa_drv_get_mac_addr(wpa_s),
						ETH_P_WAI,
						wapi_asue_rx_wai, wpa_s, 0);
		wpa_printf(MSG_DEBUG, "%s : wpa_s->l2_wapi = %p ", __FUNCTION__, wpa_s->l2_wapi);
#endif
