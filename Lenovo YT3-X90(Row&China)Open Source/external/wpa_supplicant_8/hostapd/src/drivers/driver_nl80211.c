...
#ifdef WAPI //add by xizhang for 5.0
	} else if(params->wpa_ie[0] == WLAN_EID_WAPI) {
		wpa_printf(MSG_ERROR, "## its EID_WAPI\n");
		wpa_printf(MSG_DEBUG, " * skip group cipher configuration for wapi");
#endif
	...
	struct wpa_driver_associate_params *params)
{
	...
	wpa_printf(MSG_DEBUG, " ******** nl80211_connect_common ret %d", ret);
	if (ret)
		goto nla_put_failure;
    ...
    wpa_printf(MSG_DEBUG, "  ********* params->auth_alg %d, params->wpa_ie %d", params->auth_alg, params->wpa_ie[0]);
	if (params->auth_alg & WPA_AUTH_ALG_OPEN)
		type = NL80211_AUTHTYPE_OPEN_SYSTEM;
	else if (params->auth_alg & WPA_AUTH_ALG_SHARED)
		type = NL80211_AUTHTYPE_SHARED_KEY;
