...
#endif /* PROP_TXSTATUS */
extern int wlc_set_sar(int set);
...
dhd_stop(struct net_device *net)
{
	...
	if (ifidx == 0) {
		...
		if (!dhd_download_fw_on_driverload) {
			printk("enter dhd_download_fw_on_driverload \n");
			...
			printk("out dhd_download_fw_on_driverload \n");
		}
	}
#endif /* WL_CFG80211 */

#ifdef PROP_TXSTATUS
	printk("befor dhd_wlfc_cleanup \n");
	dhd_wlfc_cleanup(&dhd->pub, NULL, 0);
#endif
	/* Stop the protocol module */
	dhd_prot_stop(&dhd->pub);
    printk("dhd_prot_stop \n");
...
static int dhd_interworking_enable(dhd_pub_t *dhd)
{
	...
	//sx9310_hook_bind(NULL);
	return ret;
}
#endif /* WL11u */

extern int get_bcm_power_pin(void);
dhd_pub_t *dhd_bk = NULL;
int wlc_set_sar(int set)
{
	int sar_enable = set;
	static int old_set = -1 ; 
	char iovbuf[32] = {0};
	if (old_set == set )
		return 0;
	old_set = set ;
    if (get_bcm_power_pin() == FALSE)
		return 0;
	if(dhd_bk&&dhd_bk->up) {
		bcm_mkiovar("sar_enable", (char *)&sar_enable, 4, iovbuf, sizeof(iovbuf));

		if (dhd_wl_ioctl_cmd(dhd_bk, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) < 0)
			DHD_ERROR(("%s: set sar_enable\n", __FUNCTION__));
	}

	printk("%s:%s():Set:%d\n",__FILE__,__FUNCTION__,set);
	return 0;
}
EXPORT_SYMBOL(wlc_set_sar);
...
#if 1 //DEBUG FS
static ssize_t bcm_sar_write(struct file *file,const char __user *user_buf,
					      size_t count, loff_t *ppos)
{
	char info[32]={0};

	copy_from_user(info,user_buf,32);
...
static const struct file_operations bcm_sar_ops={
	.owner = THIS_MODULE,
	.open = simple_open,
	.write = bcm_sar_write,
};

static int bcm_set_sar(void)
{
	printk("Init ------BCM_SET_SAR\n");
	(void) debugfs_create_file("bcm_set_sar",S_IFREG|S_IRUGO,NULL,NULL,&bcm_sar_ops);
	return 0;
}
#endif
