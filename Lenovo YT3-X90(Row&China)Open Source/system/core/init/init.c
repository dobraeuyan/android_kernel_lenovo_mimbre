...
#define LENOVO_ONEIMAGE_SUPPORT
...
static bool isAK(char* ccode) {
    ...
    ERROR("clarence -- func isAK \n");
    property_get("ro.lenovo.tablet", value);
    ERROR("clarence -- func isAK ro.lenovo.tablet=%s\n", value);
...
static int setcc_init_action(int nargs, char **args)
{
    ...
    isAK("us");
    fd = open("/country/easyimage_cc", O_RDONLY);
    if(fd < 0)
    {
        ...
        close(fd);
        if(length <= 0){
          ERROR("fail to read /country/easyimage\n");
            return 0;  
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if (is_charger) {
        ...
        action_for_each_trigger("load_all_props_action", action_add_queue_tail);
    }
    ...
    /* Lenovo-sw, [BLADEFHD-20], qianxl1, oneImage, start */
#ifdef LENOVO_ONEIMAGE_SUPPORT
    ERROR("clarence -- add setcc_init function \n");
