...
static int hostapd_ctrl_iface_tx_power(struct hostapd_iface *iface,
				       int tx_power)
{
	if (tx_power < 0)
		return -1;

	iface->ext_tx_power = tx_power;
	hostapd_set_tx_power(iface);
	return 0;
}
...
#endif /* RADIUS_SERVER */
	} else if (os_strncmp(buf, "TX_POWER ", 9) == 0) {
		if (hostapd_ctrl_iface_tx_power(hapd->iface, atoi(buf + 9)))
			reply_len = -1;
	} else {
...
int hostapd_ctrl_iface_init(struct hostapd_data *hapd)
{
	...
	wpa_printf(MSG_DEBUG,
		   "hostapd_ctrl_iface_init addr.sun_path %s, s: %d",
		   addr.sun_path, s);
	if (s >= 0)
		goto havesock;
#endif /* ANDROID */
