...
static int dpm_suspend_noirq(pm_message_t state)
{
	...
	while (!list_empty(&dpm_late_early_list)) {
		...
		if (strcmp(dev_driver_string(dev), "iTCO_wdt") != 0)
		...
		if (!list_empty(&dev->power.entry)) {
			if (strcmp(dev_driver_string(dev), "iTCO_wdt") != 0)
				...
				list_move(&dev->power.entry, &dpm_wdt_list);
		}
		put_device(dev);
	...
	while (!list_empty(&dpm_wdt_list)) {
		struct device *dev = to_device(dpm_wdt_list.prev);
