...
static void ftrace_dump_gsm_data( char* tty_name, int is_out,
                                         const void* buf, size_t len )
{
    ...
    if ( tty_name == NULL ) return;

    if ( len > 300 ) {
        printk("[%s %s] %d bytes skipped.\n", tty_name,
            is_out == 1 ? "-->" : "<--", len);
        return;
    }
...
/*
                        tty = tty_port_tty_get(port);
                        if (tty != NULL) {
                            if(0 == strncmp("gsmtty10", tty->name, 8))
                                ftrace_dump_gsm_data(tty->name, 0, data, len);
                        }
*/
...
/*
	...
	{
        if(0 == strncmp("gsmtty10", tty->name, 8))
            ftrace_dump_gsm_data(tty->name, 1, buf, len);
	}
*/
