				...
				struct usb_device *udev)
{
	...
	if (is_ssic_roothub(ssic, udev)) {
		...
		dev_err(ssic->dev, "%s: SSIC roothub added\n", __func__);
		...
		dev_err(ssic->dev, "%s: SSIC device added\n", __func__);
	}
}
				...
				struct usb_device *udev)
{
	...
	if (is_ssic_roothub(ssic, udev)) {
		...
		dev_err(ssic->dev, "%s: SSIC roothub removed\n", __func__);
		...
		dev_err(ssic->dev, "SSIC device removed\n");
	}
}
