...
static inline int bq2589x_enable_charger(struct bq2589x *bq, bool val)
{
	...
	bq2589x_write_byte(g_bq,0x05,0x73);
	bq2589x_write_byte(g_bq_sec,0x05,0x73);
	...
	if((bq->main_charger_flag == 1)&&(sch_flag==0)&&(val==1)&&(bq->usb.type==6))
                {
                        sch_flag=1;
			printk("liulc1=start==chrg_boost_wrkr\n");
...
static void bq2589x_boost_worker(struct work_struct *work)
{
	...
	v1=read_bat(1)/1000;
	v2=read_bat(2)/1000;
	printk("liulc1=================main_bat=%d sec_bat=%d\n",v1,v2); 
...
int charger_temp(int y)
{
	...
	printk("liulc1 charger_temp   x=%d  w=%d  z=%d\n",x,w,z);	
	for(i=0;i<181;i++)
	{
		if(x>=R[i]*100)
			return T[i]*10;
...
void config_bq25892(void)
{
		...
		if(yt3_hw_ver!=0)
		{
			...
			printk("==liulc2===y1=%d y2=%d charger_main_temp=%d charger_sec_temp=%d\n", y1,y2,charger_main,charger_sec);
		}
#endif
		...
		vbus=bq2589x_adc_read_vbus_volt();
        	printk("===liulc1====1-vbus=%d  t1=%d t2=%d\n",vbus,t1,t2);
...
static int bq2589x_turn_otg_vbus(struct bq2589x *bq, bool votg_on)
{
	...
	if (votg_on) {
			if(!wake_lock_active(&bq->wakelock))
	                        wake_lock(&bq->wakelock);
			 if(yt3_hw_ver==0)
				gpio_direction_output(388, 1);
			else
				gpio_direction_output(336, 1);	
			...
			if(yt3_hw_ver==0)
                                gpio_direction_output(388, 0);
                        else    
                                gpio_direction_output(336, 0);
...
static int bq2589x_init_main(struct bq2589x *bq)
{
	...
	bq2589x_write_byte(bq,0x05,0x73);
...
static int bq2589x_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("=========liulc1=======bq2589x_charger_probe   yt3_hw_ver= %d\n",yt3_hw_ver);
	if(bq->main_charger_flag == 0)
	{
		if(yt3_hw_ver==0)
		{
			ret = gpio_request(388, "bq2589x chip-enable pin");
               		 if (ret) {
                        	printk("bq2589x_charger_probe  gpio request failed 388\n");
                	        goto err_dev;
        	        }
	                gpio_direction_output(388, 0);  
		}
		...
		{
			ret = gpio_request(336, "bq2589x chip-enable pin");
                         if (ret) {
                        	printk("bq2589x_charger_probe  gpio request failed 336\n");
                                goto err_dev;
                        }
                        gpio_direction_output(336, 0);  
