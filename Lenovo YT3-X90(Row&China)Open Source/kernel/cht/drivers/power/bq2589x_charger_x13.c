...
#include <linux/power/bq2589x_reg_x13.h>
...
#include "power_supply.h"  //liulc1  add
...
static struct bq2589x *g_bq_sec;
...
int bq2589x_set_vindpm(struct bq2589x *bq, int volt);//liulc1  add
//liulc1 add
...
static enum bq2589x_vbus_type bq2589x_get_vbus_type(struct bq2589x *bq)
{
	...
	val=bq->usb.type;
	printk("===liulc2=bq2589x_get_vbus_type==type=%d\n",val);	
	return  val;
#if 0
...
static inline int bq2589x_set_cc(struct bq2589x *bq, int curr)
{
	...
	if((type==4)&&(curr!=0))
		curr=500;
	printk("==bq2589x_set_cc=liulc2===type=%d curr=%d\n",type,curr);
...
static inline int bq2589x_set_cv(struct bq2589x *bq, int volt)
{
	...
	if(volt==0)
	{
		...
		boost_flag=0;
        	sch_flag=0;
        	printk("===liulc2====cancel sch_flag=%d\n",sch_flag);
        	cancel_delayed_work_sync(&bq->chrg_boost_wrkr);

	}

	printk("==bq2589x_set_cv=liulc2===type=%d volt =%d\n",type,volt);
...
printk("==liulc2==bq2589x_enable_charging val=%d\n",val);

	dev_warn(&bq->client->dev, "%s:%d\n", __func__, val);

	ret = bq2589x_program_timers(bq, BQ2589X_WDT_160S, false);
	if (ret < 0) {
...
{
	int ret = 0;
printk("==liulc2==bq2589x_enable_charger val=%d\n",val);
	...
	if((sch_flag==0)&&(val==1))
                {
                        sch_flag=1;
			printk("liulc2=start==chrg_boost_wrkr\n");
             ...
             enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CABLE_TYPE:
		...
		bq->usb.type = get_power_supply_type(bq->cable_type);
		printk("bq2589x_usb_set_property==liulc2==bq->usb.type = %d\n",bq->usb.type);
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_CURRENT:
		ret = bq2589x_set_cc(bq, val->intval);
		if (!ret)
			bq->cc = val->intval;
		break;
	case POWER_SUPPLY_PROP_CHARGE_VOLTAGE:
		ret = bq2589x_set_cv(bq, val->intval);
		if (!ret)
			bq->cv = val->intval;
		break;
	case POWER_SUPPLY_PROP_CHARGE_TERM_CUR:
		ret = bq2589x_set_iterm(bq, val->intval);
		if (!ret)
			bq->iterm = val->intval;
		break;
	...
	case POWER_SUPPLY_PROP_INLMT:
		ret = bq2589x_set_inlmt(bq, val->intval);
		if (!ret)
			bq->inlmt = val->intval;
		break;
	default:
...
int bq2589x_set_vindpm(struct bq2589x *bq, int volt)
{
	...
	printk("===========liulc2======bq2589x_set_vindpm =%d\n",volt);
...
static void bq2589x_boost_worker(struct work_struct *work)
{
	...
	printk("==liulc2===bq2589x_boost_worker\n");

	vbus=bq2589x_adc_read_vbus_volt();
        printk("===liulc2====1-vbus=%d\n",vbus);


        if((bq->usb.type == 6)&&(vbus < 6000)&&(boost_flag==0))
        {
	...
	if(bq->usb.type == 6)
	{
		if(vbus>11000)
		{
                    bq2589x_set_cc(bq,3000);
		    bq2589x_set_inlmt(bq,2000);		
		    bq2589x_set_vindpm(bq,11500);
		}
                else if(vbus>8000&&vbus<11000)
		{
                    bq2589x_set_cc(bq,3000);
		    bq2589x_set_inlmt(bq,2000);		
		    bq2589x_set_vindpm(bq,8500);
	 ...
	 schedule_delayed_work(&bq->chrg_boost_wrkr, 10*HZ);
...
static int bq2589x_update_charge_params(struct bq2589x *bq)
{
	...
	if(type == BQ2589X_VBUS_OTG)
        return 0;
printk("bq2589x_update_charge_params==liulc2 type=%d\n",type);
