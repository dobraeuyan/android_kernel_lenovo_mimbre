...
void	process_recv(void *recv_buf, size_t data_len)
{
	...
	do {
		...
		if (cur_pos + payload_len + sizeof(struct hostif_msg) >
				total_len) {
			dev_dbg(NULL, "[hid-ish]: BAD packet [cur_pos=%u]\n",
				cur_pos);
			dev_dbg(NULL, "[hid-ish]: payload_len=%u\n",
				payload_len);
			dev_dbg(NULL, "[hid-ish]: multi_packet_cnt=%u\n",
				multi_packet_cnt);
			dev_dbg(NULL, "[hid-ish]: is_response=%02X\n",
		...
		default:
			dev_dbg(NULL, "[hid-ish]: BAD packet [cmd=%02X]\n",
				recv_msg->hdr.command);
			break;

		case HOSTIF_DM_ENUM_DEVICES:
			ISH_DBG_PRINT(KERN_ALERT
				"[hid-ish]: %s(): HOSTIF_DM_ENUM_DEVICES\n",
				__func__);
