...
void recv_fixed_cl_msg(struct heci_device *dev, struct heci_msg_hdr *heci_hdr)
{
	...
	if (heci_hdr->me_addr == HECI_SYSTEM_STATE_CLIENT_ADDR) {
		...
		if (msg_hdr->cmd == SYSTEM_STATE_STATE_CHANGE_REQ) {
			struct ish_system_states_state_change_req * msg = 
			msg_hdr;
			/*Android mask is set*/
			if ((msg->requested_states & ANDROID_EVENT_MASK) ==
					ANDROID_EVENT_MASK)
				if ((msg->states_status & ANDROID_EVENT_MASK) ==
						KICKSTAND_OPEN_EVENT) {
					dev->print_log(dev,
						"got KICKSTAND_OPEN_EVENT\n");
					printk("%s got KICKSTAND_OPEN_EVENT\n",__func__);
					goto out; /* discard msg */
				}
		}
