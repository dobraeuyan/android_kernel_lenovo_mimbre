...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	//static unsigned int last_power_key;
	//static unsigned int current_power_key;
	...
	if (!!state == 1) {
		...
		if (bdata->button->code == KEY_DLP) {
           ...
           /* if(dlp_inited == 0){
			    simulate_valleyview_irq_handler(!(dual_display_status.external_status == PIPE_ON), NULL);
			    return IRQ_HANDLED;
            }*/
		}
	}
	...
	if (!!state == 1) {
		if (bdata->button->code == KEY_POWER) {
			current_power_key = jiffies_to_msecs(jiffies);
			if (current_power_key - last_power_key < 200)
				return;
			last_power_key = jiffies_to_msecs(jiffies);
		}
	}
#endif
...
static irqreturn_t gpio_keys_gpio_isr(int irq, void *dev_id)
{
    ...
    if (bdata->button->code == KEY_VOLUMEUP && trigger_panic) {
        if (state) { /*key pressed*/
            vol_up_pressed = true;
            panic_start = jiffies;
            printk(KERN_INFO "Key: gpio %d, code %d pressed at %ul\n",
                    bdata->button->gpio, bdata->button->code, panic_start);
        } else { /* key released */
            bool is_timeout = false;
            unsigned long now = jiffies;
            printk(KERN_INFO "Key: gpio %d, code %d released at %ul, (start = %ul)\n",
                    bdata->button->gpio, bdata->button->code, now, panic_start);
            if (!vol_up_pressed) {
