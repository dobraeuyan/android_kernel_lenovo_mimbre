...
{
	//struct isl9860 *lp = i2c_get_clientdata(cl);
       //misc_deregister(&isl9860_dev);
	dev_warn(&cl->dev, " isl9860_remove enter exit 0");
	//sysfs_remove_group(&lp->dev->kobj, &isl9860_attr_group);
	return 0;
}
...
//module_i2c_driver(isl9860_driver);


//static struct isl9860_rom_data isl9860_rom_data[1] = {0x10, 0x84};
...
static int __init isl9860_i2c_platform_init(void)
{
	...
	strncpy(i2c_info.type, "isl9860", strlen("isl9860"));
...
#if 0
    ret = i2c_register_board_info(i2c_busnum, &isl9860_i2c_device, 1);
	if(ret) {
		printk("[drm]%s: isl9860 i2c device register 228or %d.\n", __func__, ret);
	}
#else
...
//fs_initcall(isl9860_i2c_platform_init);
