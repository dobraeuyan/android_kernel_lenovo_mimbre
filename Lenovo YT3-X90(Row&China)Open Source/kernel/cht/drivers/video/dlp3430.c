...
{    
    ...
    if(dlpdata==NULL)
		return 1;   
#if 0
	   gpio_request(204, "dlp_power_on1");
        /* FIX ME. Set DLP power up */
	gpio_direction_output(204,1);
        msleep(20);
        gpio_request(205, "dlp_power_on2");
	gpio_direction_output(205,1);
        msleep(200);
...
#else
#if 0
		vprog = regulator_get(dlpdata->dev, "DLP_PMU");
		if(IS_ERR(vprog)) {
			printk(KERN_ERR "[drm]%s: can't get regulator DLP_PMU.\n", __func__);
			return 1;
		}
		ret = regulator_enable(vprog);
		if(ret) {
			printk(KERN_ERR "[drm]%s: regulator VPROG6A enable failed. ret = %d.\n", __func__, ret);
			return 1;
		}
#endif
#if 0
		printk(KERN_ERR "[drm]%s: set gpio.\n", __func__);
		gpio_request(154, "dlp_power_on");
//		gpio_direction_output(154, 1);
...
{	
	...
	data[1] = 0xff;
	data[2] = 0;
	if (1 != dlp3430_read_reg(dlpdata, data[0], &data[1], 1)) {
		printk(KERN_ERR "[drm] %s read device ID failed.\n", __func__);
	} else {
		printk(KERN_ERR "[drm] %s read device ID is 0x%x.\n", __func__, data[1]);
	}
#endif
#ifndef	BLADEIII_10A_DLP_PO
	printk(KERN_ERR "----%s ----\n",__func__);
...
#else
	//gpio_request_one(156, GPIOF_OUT_INIT_HIGH,"dlp_power_on");
#endif
