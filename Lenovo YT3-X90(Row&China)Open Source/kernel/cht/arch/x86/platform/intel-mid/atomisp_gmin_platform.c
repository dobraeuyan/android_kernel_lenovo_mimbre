		...
		enum atomisp_bayer_order csi_bayer)
{
	...
	if(client) {
		if (!strcmp(client->name, "INT33FB:00")) {
			/*back ar1335 camera*/
			printk("%s %d ar1335 sensor found:%lx \r\n", __func__, __LINE__, (unsigned long int)ar1335_platform_data);
			gs->csi_port = ATOMISP_CAMERA_PORT_PRIMARY;
			return ar1335_platform_data(info);
		} else if (!strcmp(client->name, "INT33BE:01")) {
			/*front ov5693 camera*/
			printk("%s %d ov5693 sensor found:%lx \r\n", __func__, __LINE__, (unsigned long int)ov5693_platform_data);
			gs->csi_port = ATOMISP_CAMERA_PORT_SECONDARY;
			return ov5693_platform_data(info);
		}
		return &gmin_plat;
	} else
		return &gmin_plat;
...
int camera_set_pmic_power(enum camera_pmic_pin pin, bool flag)
{
	...
	if (pin == CAMERA_VPROG5B) {
			...
			intel_soc_pmic_writeb(VPROG5B_VSEL, 32);//real is 1.78 for value 32,huyj3
	}
