...
def BuildBootableImage(sourcedir, fs_config_file, info_dict=None):
  ...
  new_img = os.path.basename(sourcedir).lower()+".img"
  if info_dict.get("rsign_on",'no') == 'yes':
    cmd = ["build/tools/rsign.sh", img.name, new_img]
    print "rsign %s " % (cmd,)
    p = Run(cmd, stdout=subprocess.PIPE)
    p.communicate()
    assert p.returncode == 0, "rsign of %s image failed" % path
    with open(new_img, 'rb') as tf:
      data = tf.read()
