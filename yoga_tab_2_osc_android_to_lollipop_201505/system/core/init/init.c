...
/*[lenovo_fw,fanll4,easyimage],add*/
#ifdef LENOVO_ONEIMAGE_SUPPORT
static char CountryCode[10] = {0};
static char* CC_code = NULL;

static char client_id[32] = {0};
static char search_id[32] = {0};

#define LINE  79
#define ITEMS   (LINE*3)
#define CC_LEN 10

//Lenovo: add by baijf1 for ak property
#define AKITEMS 36
static char LocaleAKSet[AKITEMS][3]={
    "au", "bg", "ca", "cy", "cz", "dk", "gh", "gr", "hr",
    "hu", "id", "in", "is", "ke", "mu", "my", "mz", "na",
    "ng", "nl", "no", "nz", "ph", "ro", "rs", "se", "sg",
    "sl", "sk", "th", "tz", "ug", "uk", "us", "vn", "za",
};
//Lenovo: end baijf1
...
#endif
/*[lenovo_fw,fanll4,easyimage],end*/
#if BOOTCHART
...
static void export_kernel_boot_props(void)
{
    ...
    /* Add ro.board.id property */
    int fd_id;
    char lcd_size[PROP_VALUE_MAX] = "unknown";

    if ((fd_id = open("/sys/mainboard/id", O_RDONLY)) < 0) {
        ERROR("fail to open /sys/mainboard/id!\n");
    } else {
        if ((ret = read(fd_id, tmp, PROP_VALUE_MAX-1)) < 0) {
            ERROR("fail to read /sys/mainboard/id!\n");
        }

        sscanf(tmp, "%s", lcd_size);
        close(fd_id);
    }

    property_set("ro.board.id", lcd_size);
    property_set("ro.board.lcd_size", lcd_size);
}

/*[lenovo_fw,fanll4,easyimage],add*/
/* Lenovo-sw, [BLADEFHD-20], qianxl1, oneImage, start */
#ifdef LENOVO_ONEIMAGE_SUPPORT
//Lenovo: add by baijf1 to set ak property
static bool isAK(char* ccode) {
    int count = 0;
    char value[PROP_VALUE_MAX];
    property_get("ro.lenovo.tablet", value);
    if(strncmp(value, "wifi", 4) != 0) {
        return false;
    }
    for(; count < AKITEMS; count++) {
        if(strncmp(ccode, LocaleAKSet[count], 2) == 0)  {
            ERROR("clarence -- Have run to here AK CCode is %s \n", ccode);
            return true;
        }
    }
    return false;
...
static int setcc_init_action(int nargs, char **args)
{
    int fd = -1;
    int length = 0;
    int count;
    ...
    int ret;
    char value[PROP_VALUE_MAX];
    fd = open("/factory/easyimage_cc", O_RDONLY);
    if(fd < 0)
    {
        ERROR("clarence -- CC_CN non-exist \n");
        return 0;
    } else {
        length = read(fd, CountryCode, CC_LEN);
        close(fd);
        if(length <= 0){
          ERROR("fail to read /factory/easyimage\n");
            return 0;  
        }
    }
    ERROR("clarence -- Have CountryCode is %s \n", CountryCode);
    //sscanf(CountryCode,"%*[\.][=]",CCode);
    for(i=0; i<2; i++){
        CCode[i]=CountryCode[i+8];
    }
    CCode[2]='\0';
    //ERROR("clarence -- Have CCode & value is %s %s \n",CCode);
    //if(CC_code)
       // property_set("persist.clarence.cccode", CC_code);
    if(CCode[0] != 0)
    {
       property_set("ro.product.countrycode", CCode);
        ...
        if(isAK(CCode)) {
            //
            ERROR("clarence -- it is AK version, so set to 1 \n");
            property_set("ro.preload.ak", "1");
        } else {
            ERROR("clarence -- it is not AK version, so set to 0 \n");
            property_set("ro.preload.ak", "0");
        }
        //Lenovo: end baijf1
       ret = property_get("persist.sys.easyimage", value);
            property_set("ro.product.countrycode", CCode);//modify zhaoll8
       if(property_get("persist.sys.easyimage", value) == 0){
            for(count=0; count<ITEMS; count=count+3) {
                if(strncmp(CCode, LocaleSet[count],2) == 0)  {
                   ...
                   property_set("persist.sys.language",LocaleSet[count+1]);
                    ...
                    property_set("persist.sys.easyimage", "1");
...
#endif
/* Lenovo-sw, [BLADEFHD-20], qianxl1, oneImage, end */
/*[lenovo_fw,fanll4,easyimage],end*/
...
int main(int argc, char **argv)
{
    ...
    /* Lenovo-sw, [BLADEFHD-20], qianxl1, oneImage, start */
#ifdef LENOVO_ONEIMAGE_SUPPORT
    queue_builtin_action(setcc_init_action, "setcc_init");
#endif
    ERROR("init start loop\n");
    /* Lenovo-sw, [BLADEFHD-20], qianxl1, oneImage, end */
/*[lenovo_fw,fanll4,easyimage],end*/
    for(;;) {
        int nr, i, timeout = -1;
