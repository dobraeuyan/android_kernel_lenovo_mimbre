...
PUB_KKTOLFILES := $(PUBLISH_TARGET)/flash_files/kktol

ifeq ($(LENOVO_EASYIMAGE_ON),yes)
FLASHFILES_CONFIG ?= $(COMMON_PATH)/default_flashfiles_ROW.json
else
FLASHFILES_CONFIG ?= $(COMMON_PATH)/default_flashfiles_PRC.json
endif

BLANK_FLASHFILES_CONFIG ?= $(COMMON_PATH)/default_blankflashfiles.json
OTA_CONFIG ?= $(COMMON_PATH)/default_ota_flashfiles.json
KKTOL_FLASHFILES_CONFIG ?= $(COMMON_PATH)/kk_to_l_flashfiles.json
...
$(eval KKTOL_T2F := $(shell ./vendor/intel/support/flashdep.py $(KKTOL_FLASHFILES_CONFIG)))
...
KKTOL_IMAGES := $(foreach dep,$(KKTOL_T2F),$(word 2,$(subst :, ,$(dep))))
...
KKTOL_XML := $(shell ./vendor/intel/support/flashtarget.py $(KKTOL_FLASHFILES_CONFIG))
KKTOL_XML := $(addprefix $(PUB_KKTOLFILES)/,$(KKTOL_XML))
FIRST_KKTOL_XML := $(firstword $(KKTOL_XML))
OTHER_KKTOL_XML := $(wordlist 2,9,$(KKTOL_XML))

# Flashfiles
.PHONY: flashfiles
flashfiles: firmware fastboot_flashfile
...
blank_flashfiles: firmware $(PUB_BLANK_ZIP)

# KK_to_L
$(FIRST_KKTOL_XML): $(KKTOL_FLASHFILES_CONFIG) $(KKTOL_IMAGES)
	$(hide) mkdir -p $(@D)
	$(hide) ./vendor/intel/support/flashxml.py -c $< -p $(REF_PRODUCT_NAME) -b $(FILE_NAME_TAG) -d $(@D) -t "$(KKTOL_T2F)"

$(OTHER_KKTOL_XML): $(FIRST_KKTOL_XML)

.PHONY: pub_kktol_flashfiles
pub_kktol_flashfiles: $(KKTOL_IMAGES) | $(KKTOL_XML) $(ACP)
	@echo copying kktol_flashfiles: $(notdir $^)
	$(hide) mkdir -p $(PUB_KKTOLFILES)
	$(hide) $(ACP) $^ $(PUB_KKTOLFILES)

PUB_KKTOL_ZIP := $(PUB_KKTOLFILES)/$(TARGET_PRODUCT)-$(TARGET_BUILD_VARIANT)-kktol-$(FILE_NAME_TAG).zip
$(PUB_KKTOL_ZIP): $(KKTOL_IMAGES) $(KKTOL_XML)
	$(hide) mkdir -p $(@D)
	$(hide) rm -f $@
	@echo generating $@
	$(hide) zip -j $@ $^
...
.PHONY: kktol_flashfiles
kktol_flashfiles: firmware $(PUB_KKTOL_ZIP)
