...
DxDI_RetCode_t DxDI_GetAbiVersion(uint32_t *verMajor, uint32_t *verMinor)
{
	...
	if (verMinor != NULL) *verMinor = abiVerMinor;  // INTEL change
	return DXDI_RET_OK;
}
...
)
{
	...
	if (rc == DXDI_RET_OK) {
		...
		if ((params.digest_or_mac_size > 0) &&  /* Get digest/MAC results */
		    (digestOrMacSize != NULL))  // INTEL change; Klockwork fix
			memcpy(digestOrMac, &(params.digest_or_mac),
				*digestOrMacSize);
	}
...
)
{
	...
	params.context_buf = contextBuf;
	memcpy(&(params.props), props, sizeof(struct dxdi_auth_enc_props)); // INTEL change
	params.adata = aData;
 ...
 * 
 * \param uuid The UUID of the applet that is being connected to
 * \param contextP A reference to a context data buffer initialized with DxDI_SepAppContextAlloc 
 * \param sessionId A session ID as return from DxDI_SepAppSessionOpen
...
DxDI_RetCode_t DxDI_SepAppCommandInvoke(
	dxdi_sepapp_uuid_t uuid,  //INTEL Change
	DxDI_SepAppContext_t *contextP,
	dxdi_sepapp_session_id_t sessionId,
	uint32_t commandId,
	...
	uint32_t *errorInfo)
{
	...
	memcpy(params.app_uuid, uuid, sizeof(dxdi_sepapp_uuid_t));  //INTEL Change
	params.session_id = sessionId;
