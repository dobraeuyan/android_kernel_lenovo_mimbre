...
/* Generated by utils/src/sep_rpc_gen/sep_rpc_gen.py@3343 at 2014-08-21 19:28:34.470000 */
/******************************************************************** 
	...
	DxUint32_t EphemerKeyData_ptr[])
{
	...
	} else if (SignatureOutSize_ptr != NULL) {  // INTEL change; Klockwork fix
	...
	DxBool_t isExactMessageInSize)
{
	...
	params.isExactMessageInSize = cpu_to_le32(isExactMessageInSize);
	...
	} else if (EncrMessOutSize_ptr != NULL) {   // INTEL change; Klockwork fix
...
/************ _DX_ECPKI_ELGAMAL_Decrypt ***************/
CRYSError_t _DX_ECPKI_ELGAMAL_Decrypt(
	...
	DxBool_t isExactMessageInSize)
{
	DxDI_RetCode_t diRc;
	seprpc_retcode_t rpcRc;
	struct SepRpc__DX_ECPKI_ELGAMAL_DecryptParams params;
	...
	} else if (DecrMessageOutSize_ptr != NULL) {    // INTEL change; Klockwork fix
	...
	params.num_of_memrefs = cpu_to_le32(SEPRPC_MEMREF_NUM__DX_ECPKI_ELGAMAL_Decrypt);

	diRc = DxDI_SepRpcCall(SEPRPC_AGENT_ID_CrysEcc, SEPRPC_FUNC_ID__DX_ECPKI_ELGAMAL_Decrypt,
		NULL, sizeof(params), (struct seprpc_params*)&params, &rpcRc);

	SEP_RPC_ASSERT(diRc == DXDI_RET_OK, CRYS_ECC_HOST_MSG_GENERAL_RPC_A_ERROR);
	if (rpcRc != SEPRPC_RET_OK) {
		SEP_RPC_LOG("RPC of _DX_ECPKI_ELGAMAL_Decrypt failed with RPC error code %d\n", rpcRc);
		switch (rpcRc) {
		case SEPRPC_RET_EINVAL:
			return CRYS_ECC_HOST_MSG_GENERAL_RPC_B_ERROR;
		case SEPRPC_RET_ENORSC:
	...
	CRYS_ECDH_TempData_t *TempBuff_ptr)
{
	...
	} else if (SharedSecrValSize_ptr != NULL) { // INTEL change; Klockwork fix
		/* Verify array size of SharedSecretValue_ptr */
		SEP_RPC_ASSERT((*SharedSecrValSize_ptr) <= (2*4*CRYS_ECPKI_MODUL_MAX_LENGTH_IN_WORDS), CRYS_ECC_HOST_MSG_GENERAL_RPC_B_ERROR);
	...
	DxUint32_t *PublKeySizeInBytes_ptr)
{
	...
	} else if (PublKeySizeInBytes_ptr != NULL) {    // INTEL change; Klockwork fix
...
/************ CRYS_ECPKI_GenKeyPairUserSeed ***************/
CRYSError_t CRYS_ECPKI_GenKeyPairUserSeed(
	CRYS_ECPKI_DomainID_t DomainID,
	CRYS_ECPKI_UserPrivKey_t *UserPrivKey_ptr,
	CRYS_ECPKI_UserPublKey_t *UserPublKey_ptr,
	CRYS_ECPKI_KG_TempData_t *TempData_ptr,
	DxUint8_t userSeed_ptr[],
	DxUint32_t userSeedSizeInBytes)
{
	DxDI_RetCode_t diRc;
	seprpc_retcode_t rpcRc;
	struct SepRpc_CRYS_ECPKI_GenKeyPairUserSeedParams params;

	/* Verify input parameters and copy into params. struct. */
	params.DomainID = cpu_to_le32(DomainID);
	params.TempData_ptr_size = cpu_to_le32((sizeof(CRYS_ECPKI_KG_TempData_t)));
	if (userSeed_ptr == NULL) {
		params.userSeed_ptr_null_flag = 1;
	} else {
		/* Verify array size of userSeed_ptr */
		SEP_RPC_ASSERT((userSeedSizeInBytes) <= (CRYS_ECPKI_USER_SEED_SIZE_IN_BYTES), CRYS_ECC_HOST_MSG_GENERAL_RPC_B_ERROR);
		memcpy(params.userSeed_ptr, userSeed_ptr, sizeof(DxUint8_t) * (userSeedSizeInBytes));
		params.userSeed_ptr_null_flag = 0;
	}

	params.userSeedSizeInBytes = cpu_to_le32(userSeedSizeInBytes);
	if (UserPrivKey_ptr == NULL) {
		params.__UserPrivKey_ptr_null_flag = 1;
	} else {
		params.__UserPrivKey_ptr_null_flag = 0;
	...
	params.num_of_memrefs = cpu_to_le32(SEPRPC_MEMREF_NUM_CRYS_ECPKI_GenKeyPairUserSeed);

	diRc = DxDI_SepRpcCall(SEPRPC_AGENT_ID_CrysEcc, SEPRPC_FUNC_ID_CRYS_ECPKI_GenKeyPairUserSeed,
		NULL, sizeof(params), (struct seprpc_params*)&params, &rpcRc);

	SEP_RPC_ASSERT(diRc == DXDI_RET_OK, CRYS_ECC_HOST_MSG_GENERAL_RPC_A_ERROR);
	if (rpcRc != SEPRPC_RET_OK) {
		SEP_RPC_LOG("RPC of CRYS_ECPKI_GenKeyPairUserSeed failed with RPC error code %d\n", rpcRc);
		switch (rpcRc) {
		case SEPRPC_RET_EINVAL:
			return CRYS_ECC_HOST_MSG_GENERAL_RPC_B_ERROR;
		case SEPRPC_RET_ENORSC:
