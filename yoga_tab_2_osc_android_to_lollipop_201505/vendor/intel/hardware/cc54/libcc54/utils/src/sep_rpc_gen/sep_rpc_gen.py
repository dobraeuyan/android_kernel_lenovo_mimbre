...
********************************************************************/\n""")
    ...
    gen_rev = re.sub(r"\$Rev:\s*([0-9]+)\s*\$", r"\1", r"$Rev: 3343 $")
...
def gen_sep_c_file(api_data, base_name, c_file_path):
    ...
    sep_c_file.write("#define DX_PAL_LOG_CUR_COMPONENT DX_LOG_MASK_RPC\n\n")

    # includes
    for header in api_data["headers"] :
        sep_c_file.write("#include \"" + header + "\"\n")
    sep_c_file.write("#include \"dx_pal_log.h\"\n")
    ...
    for function in api_data["functions"] :
        ...
        sep_c_file.write("\t\t\tDX_PAL_LOG_ERR(\"Invalid HMB size for struct SepRpc_" + func_name + "Params\\n\");\n")
