...
/**
 ...
 @param[out] SharedSecretValue_ptr      A pointer to an output buffer that will contain
  ...
  @param[in,out] SharedSecrValSize_ptr   A pointer to the size of user passed buffer (in) and 
        				...
        				If the  user need output the shared secret value as
        				one X-coordinate he must set buffer size as follows
					2*(modulus size) > SharedSecrValSize >= (modulus size);
					if he need two coordinates concatenated as X || Y,
					he must set the SharedSecrValSize >= 2*(modulus size).
