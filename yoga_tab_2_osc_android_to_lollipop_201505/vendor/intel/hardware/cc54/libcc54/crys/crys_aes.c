				     ...
				     CRYS_AES_OperationMode_t OperationMode)
{
	...
	struct dxdi_sym_cipher_props aes_cipher_props = {0};    // INTEL change; Klocwork fix
	struct dxdi_mac_props aes_mac_props = {0};  // INTEL change; Klocwork fix
...
{    
	...
	if ((OperationMode == CRYS_AES_XTS_mode) && (DataInSize < CRYS_AES_BLOCK_SIZE_IN_BYTES)) {
			return CRYS_AES_DATA_IN_SIZE_ILLEGAL;
	}
