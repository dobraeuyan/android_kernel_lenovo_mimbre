...
{                              
	...
	CRYSError_t   crysRc;
	DxUint32_t *contextPtr;
	DxUint8_t tag[CRYS_AESCCM_MAC_MAX_SIZE_BYTES] = { 0 };
	...
	if (diRc == 0) {
		struct dxdi_auth_enc_props *AesCcmProps =
				(struct dxdi_auth_enc_props *)ContextID_ptr;

		if (AesCcmProps->direction == DXDI_CDIR_DEC) {  // INTEL change; fix warning
			/* Check mac result */
			if (DX_PAL_MemCmp(MacRes, tag, *SizeOfT)) {
				return CRYS_AESCCM_CCM_MAC_INVALID_ERROR;
			}
		} else { /*CRYS_AES_Encrypt*/
			DX_PAL_MemCopy(MacRes, tag, *SizeOfT);
		}
	}
	...
	CRYS_AESCCM_Mac_Res_t        MacRes)
{
	...
	if (diRc == CRYS_OK) {
		if (EncrDecrMode == CRYS_AES_Decrypt) {
			/* Check mac result */
			if (DX_PAL_MemCmp(MacRes, tag, aesccm_cipher_props.tag_size)) {
				return CRYS_AESCCM_CCM_MAC_INVALID_ERROR;
			}
		} else { /*CRYS_AES_Encrypt*/
			DX_PAL_MemCopy(MacRes, tag, aesccm_cipher_props.tag_size);
		}
	}
