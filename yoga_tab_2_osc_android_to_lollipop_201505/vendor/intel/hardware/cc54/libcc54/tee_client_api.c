	...
	uint32_t        *returnOrigin)
{
	...
	memcpy(diSessionP->uuid, (dxdi_sepapp_uuid_t *)destination, sizeof(diSessionP->uuid)); //INTEL Change
	...
	uint32_t         *returnOrigin)
{
	...
	diRc = DxDI_SepAppCommandInvoke(diSessionP->uuid, diSessionP->contextP,
					diSessionP->sessionId, commandID,
					sepAppParamsP, &sepRetOrigin,
					&sepRetCode); //INTEL Change
	if (diRc == DXDI_RET_OK) {
		CopyBackValParams(operation, sepAppParamsP);
	}
	*returnOrigin = sepModuleToTeecOrigin(sepRetOrigin);
