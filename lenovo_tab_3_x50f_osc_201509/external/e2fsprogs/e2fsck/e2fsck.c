...
errcode_t e2fsck_allocate_context(e2fsck_t *ret)
{
	...
	if (time_env)
		...
		context->now = time(0);
#if 0
	/* update the timestamp even if the tiem appears insane, this is
	 * for the reason that resize2fs will need the fs be checked and clean.
	 * if we keep the timestamp on 1970.1.1, resize2fs will refuse to work.
	 */
