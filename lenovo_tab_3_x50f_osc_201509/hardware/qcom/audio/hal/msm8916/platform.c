...
}; //added by yuansf for saparate SPK
#endif
...
#endif
#ifdef AUDIO_SELECT_SPEAKER
    int select_spk; //added by yuansf for saparate spk
#endif
    void *hw_info;
    struct csd_data *csd;
};
...
static int acdb_device_table[SND_DEVICE_MAX] = {
...
#ifdef CONFIG_LCT_MSM8909_P5000_PCB
    ...
    [SND_DEVICE_OUT_SPEAKER_ONLY_LEFT] = 14,
    [SND_DEVICE_OUT_SPEAKER_ONLY_RIGHT] = 14,
#endif
...
struct snd_device_index snd_device_name_index[SND_DEVICE_MAX] = {
    ...
    {TO_NAME_INDEX(SND_DEVICE_OUT_SPEAKER_ONLY_LEFT)},
    {TO_NAME_INDEX(SND_DEVICE_OUT_SPEAKER_ONLY_RIGHT)},
...
#ifdef AUDIO_SELECT_SPEAKER
    my_data->select_spk = FACTORY_SELECT_SPK_NONE; //added by yuansf for saparate spk
#endif
...
#endif
        {
        #ifndef AUDIO_SPEAKER_SWAP_BY_ROTATION
            if (adev->speaker_lr_swap)
                snd_device = SND_DEVICE_OUT_SPEAKER_REVERSE;
            else {
        #endif
        #ifdef AUDIO_SELECT_SPEAKER
                if (my_data->select_spk == FACTORY_SELECT_SPK_NONE)
...
#ifdef AUDIO_SPEAKER_SWAP_BY_ROTATION
void platform_swap_rx1_rx2_mixer(void *platform, bool swap)
{
    struct platform_data *my_data = (struct platform_data *)platform;
    struct audio_device *adev = my_data->adev;
    struct mixer_ctl *ctl_rx1_mix1;
    struct mixer_ctl *ctl_rx2_mix1;
    const char *mixer_ctl_rx1_name = "RX1 MIX1 INP1";
    const char *mixer_ctl_rx2_name = "RX2 MIX1 INP1";
    const char *channel_rx1 = "RX1";
    const char *channel_rx2 = "RX2";

    ctl_rx1_mix1 = mixer_get_ctl_by_name(adev->mixer, mixer_ctl_rx1_name);
    if (!ctl_rx1_mix1) {
        ALOGE("%s: Could not get ctl for mixer cmd - %s",
              __func__, mixer_ctl_rx1_name);
    } else {
        if (!swap) {
            if (mixer_ctl_set_enum_by_string(ctl_rx1_mix1, channel_rx2)) {
                ALOGE("%s: invalid rx1 mixer string: %s", __func__, channel_rx2);
            }
        } else {
            if (mixer_ctl_set_enum_by_string(ctl_rx1_mix1, channel_rx1)) {
                ALOGE("%s: invalid rx1 mixer string: %s", __func__, channel_rx1);
            }
        }
    }

    ctl_rx2_mix1 = mixer_get_ctl_by_name(adev->mixer, mixer_ctl_rx2_name);
    if (!ctl_rx2_mix1) {
        ALOGE("%s: Could not get ctl for mixer cmd - %s",
              __func__, mixer_ctl_rx2_name);
    } else {
        if (!swap) {
            if (mixer_ctl_set_enum_by_string(ctl_rx2_mix1, channel_rx1)) {
                ALOGE("%s: invalid rx2 mixer string: %s", __func__, channel_rx1);
            }
        } else {
            if (mixer_ctl_set_enum_by_string(ctl_rx2_mix1, channel_rx2)) {
                ALOGE("%s: invalid rx2 mixer string: %s", __func__, channel_rx2);
            }
        }
    }
}
