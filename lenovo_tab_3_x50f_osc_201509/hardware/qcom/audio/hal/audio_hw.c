...
#define AUDIO_PARAMETER_KEY_SELECT_SPK  "select_spk"  //added by yuansf for saparate spk test
#endif
...
int select_devices(struct audio_device *adev, audio_usecase_t uc_id)
{
    ...
    adev->out_device = usecase->stream.out->devices;
    ...
    if (usecase->type == PCM_PLAYBACK && 
        ...
        platform_swap_rx1_rx2_mixer(adev->platform, adev->speaker_lr_swap);
    }
#endif
                                 ...
                                 const char *keys)
{
	...
	ret = str_parms_get_str(query, "routing", value,
                            sizeof(value));
    if (ret >= 0) {
        str_parms_add_int(reply, "routing", adev->out_device);
        goto exit;
    }
    pthread_mutex_lock(&adev->lock);
