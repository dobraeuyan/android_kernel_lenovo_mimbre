	  ...
	  mEnabledTime(0)
{
	...
	err = sensor_XML.read_sensors_params(context->sensor, &cal_result, CAL_DYNAMIC);
	if (err < 0) {
		ALOGE("read %s calibrate params error\n", context->sensor->name);
	}
	calibration[0] = cal_result.offset[0];
	calibration[1] = cal_result.offset[1];
	calibration[2] = cal_result.offset[2];

	accel_handle = context->sensor->handle;
	mPendingEvent.version = sizeof(sensors_event_t);
	...
	while (count && mInputReader.readEvent(&event)) {
		...
		if (type == EV_ABS) {
			...
			if (event->code == EVENT_TYPE_ACCEL_X) {
				mPendingEvent.data[0] = (value - calibration[0]) * CONVERT_ACCEL_X;
			} else if (event->code == EVENT_TYPE_ACCEL_Y) {
				mPendingEvent.data[1] = (value - calibration[1]) * CONVERT_ACCEL_Y;
			} else if (event->code == EVENT_TYPE_ACCEL_Z) {
				mPendingEvent.data[2] = (value - calibration[2]) * CONVERT_ACCEL_Z;
			} else if (event->code == ABS_MISC && value == 0) {
			    KXTJ_ResetDOE();
			    ALOGD("Calibration reset end");
			}
		} else if (type == EV_SYN) {
			switch (event->code){
				case SYN_TIME_SEC:
...
int AccelSensor::KXTJ_ResetDOE(void){
	int fd;
	int i, err;
	char temp[ARRAY][LENGTH];
	char buf[ARRAY * LENGTH];
	...
	struct SensorContext* list = sm.getInfoByHandle(accel_handle);
	if(list == NULL) {
		ALOGE("Invalid handle(%d)",accel_handle);
		return -EINVAL;
	}
	fd = open("/sys/class/input/input6/device/do_cali", O_RDWR);
	if (fd >= 0) {
		int para1 = 1;
		snprintf(buf, sizeof(buf), "%d", para1);
		err = write(fd, buf, strlen(buf)+1);
	...
	fd = open("/sys/class/input/input6/device/cali", O_RDWR);
	if (fd >= 0) {
		char *p = buf;
		memset(buf, 0, sizeof(buf));
		err = read(fd, buf, sizeof(buf)-1);
		...
		for(int i = 0; i < ARRAY; i++) {
			cal_result.offset[i] = strtol(temp[i], &endptr, 0);
			ALOGD("cal_result.offset[%d] = %d", i, cal_result.offset[i]);
			if (endptr == temp[i]) {
				ALOGE("No digits were found\n");
				return -1;
			}
		}
		cal_result.offset[2] = cal_result.offset[2] - 16384;
		err = sensor_XML.write_sensors_params(list->sensor, &cal_result, CAL_DYNAMIC);
		if (err < 0) {
			ALOGE("write calibrate %s sensor error\n", list->sensor->name);
			return err;
		}
		ALOGD("write calibrate %s sensor done\n", list->sensor->name);
