					 ...
					 sensors_event_t *la, int dt)
{
	...
	/* Orientation */
	if(ori) {
		ori->orientation.x = or_buf.orientation.x;	/* Azimuth */
		ori->orientation.y = or_buf.orientation.y; /* Pitch */
		ori->orientation.z = or_buf.orientation.z;	/* Roll */
		ori->orientation.status = or_buf.orientation.status;
	}
