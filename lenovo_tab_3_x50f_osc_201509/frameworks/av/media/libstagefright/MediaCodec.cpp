...
#ifdef DOLBY_UDC_VIRTUALIZE_AUDIO
    // Return INVALID_OPERATION error if the code is not DDP and Dolby parameters
    // are being set. This is needed becaused any errors in mCodec->signalSetParameters()
    // function are not delivered to the caller.
    int32_t enableProcessedAudio;
    if (params->findInt32(DOLBY_PARAM_PROCESSED_AUDIO, &enableProcessedAudio)) {
        if (mInitNameIsType) {
            if (!(mInitName.startsWith("audio/ac3") || mInitName.startsWith("audio/eac3"))) {
                return INVALID_OPERATION;
            }
        } else {
            if (!mInitName.startsWith("OMX.dolby")) {
                return INVALID_OPERATION;
            }
        }
    }
