...
MtpResponseCode MtpServer::doGetDeviceInfo() {
    ...
    char model_name[PROPERTY_VALUE_MAX] = "Lenovo YT3";
    ...
    if(!strncmp(prop_value, "YOGA Tablet 3", 13)) {
        strlcpy((model_name+10), (prop_value+13), strlen(prop_value));
        ALOGE("mtp server get model name is %s",model_name);
		strlcpy(prop_value, model_name, (strlen(model_name) +1));
    }
    string.set(prop_value);
...
MtpResponseCode MtpServer::doGetStorageIDs() {
	...
	ALOGV("doGetStorageIDs: mStorageCount = %d", count);
    mData.putUInt32(count);
    for (int i = 0; i < count; i++)
    {
        mData.putUInt32(mStorages[i]->getStorageID());

		ALOGV("doGetStorageIDs: StorageID[%d] = 0x%08x", i, mStorages[i]->getStorageID());
    }
    return MTP_RESPONSE_OK;
}
...
MtpResponseCode MtpServer::doGetStorageInfo() {
	...
	ALOGV("doGetStorageInfo: storage id = 0x%x", id);
    if (!storage)
        return MTP_RESPONSE_INVALID_STORAGE_ID;
	ALOGV("doGetStorageInfo: storage type = 0x%x", storage->getType());
	ALOGV("doGetStorageInfo: storage file system type = 0x%x", storage->getFileSystemType());
