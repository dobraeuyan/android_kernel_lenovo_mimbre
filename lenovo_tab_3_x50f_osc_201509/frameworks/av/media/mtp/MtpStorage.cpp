...
uint64_t MtpStorage::getMaxCapacity() {
    ...
    do {
        struct statfs   stat;
		ALOGV("MtpStorage::getMaxCapacity: getPath = %s \n", getPath());
        if (statfs(getPath(), &stat))
            return -1;
			ALOGV("MtpStorage::getMaxCapacity: stat.f_blocks = %lld, stat.f_bsize =%d \n", stat.f_blocks, stat.f_bsize);
        mMaxCapacity = (uint64_t)stat.f_blocks * (uint64_t)stat.f_bsize;
    } while (mMaxCapacity == 0);
	ALOGV("MtpStorage::getMaxCapacity: mMaxCapacity = %lld \n", mMaxCapacity);
    return mMaxCapacity;
}
...
uint64_t MtpStorage::getFreeSpace() {
	...
	ALOGV("MtpStorage::getFreeSpace: freeSpace = %lld,  mReserveSpace = %lld \n", freeSpace, mReserveSpace);
...
bool MtpStorage::setDescription(const char* description) {
	ALOGI("MtpStorage::setDescription: description = %s \n", description);
